...
#As pre-processing needs to be done only for first iteration, provide the iteration value. e.g: iteration0
...
path = os.getcwd()
print(path)
os.system('mkdir -p execution/' + iteration)
os.chdir('execution/' + iteration)
path_new = os.getcwd()
print(path_new)
...
mv = shutil.copy("../../../../datasets/ner/mv.txt", "data/mv.txt")
...
test_read = fn.read_conll("data/answers.txt")
...
for line in sentence_list:
    ...
    for line1 in sentence_loop:
        ...
        if counter > 0 and counter1 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter1, counter)
        if counter > 0 and counter2 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter2, counter)
        if counter > 0 and counter3 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter3, counter)
        if counter > 0 and counter4 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter4, counter)
        if counter > 0 and counter5 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter5, counter)
        if counter > 0 and counter6 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter6, counter)
        if counter > 0 and counter7 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter7, counter)
        if counter > 0 and counter8 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter8, counter)
        if counter > 0 and counter9 > 0:
            entropy_word_wise = entropy_word_wise - fn.entropy_calc(weight_of_worker, counter9, counter)
        if counter > 0:
